...
export const createPackageSchema = Joi.object({
  ...
  packageType: Joi.string().valid(...Object.values(PACKAGE_TYPES)).required(),
  durationDays: Joi.number().integer().min(1).required(),
  pricePerGroup: Joi.number().min(0).optional(),
...
export const packageStatusSchema = Joi.object({
  status: Joi.string().valid("draft", "pending", "published", "archived").required(),
});
